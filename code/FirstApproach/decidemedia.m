function [Agents, Cops, Map]=decidemedia(AgentsOld, CopsOld, MapOld,K,vp, Thresh, media, mediaperc, mediaimp, Size)
...
for k=1:size(AgentsToUpdate,1)                                              %Update them one at a time
   ...
   if AgentsToUpdate(k,6)==0
        Neigh=check_neigh(Map, AgentsToUpdate, k, vp, Size);                %create vector with all position within vision
        ...
        if (media==1&&AgentsToUpdate(k,7)<=mediaperc)                       %behaviour of agents with access to media if media active
