...
for t=1:maxT
   ...
   if t>1                                                                           %look at previous number of active and free people (needed for update of media term)
    ...
    [Agents, Cops, Map]=decidemedia(Agents, Cops, Grid,K,vp, Thresh, mediaperc, mediaimp, Size); %make decision whether to turn active or inactive
...
%     visualize(Grid)
    ...
    [Agents, Grid]=JailUpdate(Agents, Grid);                                        %update jail term, place agents with expiring jail term back on grid
    Nactive(t)=count_active(Agents);
    time=[time;t];
    mediapercentage=[mediapercentage;mediaperc];
%% calculate maximum value of active people during the sweep
